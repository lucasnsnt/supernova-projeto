...
export function StudentTodayPage() {
  ...
  return <div className="page-stack">
    ...
    <section><div className="section-heading"><div><p className="section-kicker">Sua programação</p><h2>Hoje</h2></div></div><div className="preview-list student-day-list">
      {todayConfirmations.map(item => <article className="preview-card" key={item.id}><header><div><span className="direction-pill">{directionLabel(item.direction)}</span><h3>{item.scheduledTime.slice(0, 5)}</h3></div><span className={`response ${item.status.toLowerCase()}`}>{confirmationLabel[item.status]}</span></header><p className="preview-destination">{item.institutionName || 'Instituição cadastrada'}</p><p className="muted preview-explanation">{item.status === 'YES' ? `Sua presença está confirmada. A rota será montada após ${time(item.responseDeadline)}.` : item.status === 'PENDING' ? `Confirme até ${time(item.responseDeadline)} para entrar na rota.` : item.status === 'NO' ? 'Você informou que não fará esta viagem.' : 'O prazo terminou sem confirmação; esta viagem não entrará na rota.'}</p></article>)}
      {!confirmations.isLoading && !todayConfirmations.length && <p className="empty-copy">Nenhum transporte previsto na sua agenda de hoje.</p>}
    </div></section>
