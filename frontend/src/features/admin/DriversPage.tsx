...
export function DriversPage() {
  ...
  return <div className="page-stack"><header className="page-heading"><p className="eyebrow">Administração</p><h1>Motoristas</h1><p className="muted">Analise e libere os motoristas que utilizarão a plataforma.</p></header><div className="card-list">{list.isLoading && <p>Carregando…</p>}{list.data?.map((driver) => <article className="trip-card large" key={driver.id}><div><span className="tag neutral">{driver.status}</span><h3>{driver.name}</h3><p className="muted">{driver.email} · CNH {driver.cnh}</p>{driver.statusReason && <p className="form-error">{driver.statusReason}</p>}</div>{driver.status === 'PENDING' && <div className="button-row"><button className="secondary-button" onClick={() => requestRejection(driver.id)}>Rejeitar</button><button className="primary-button compact" onClick={() => approve.mutate(driver.id)}>Aprovar</button></div>}</article>)}{!list.isLoading && !list.data?.length && <p className="empty-copy">Nenhum motorista cadastrado.</p>}</div></div>
}
