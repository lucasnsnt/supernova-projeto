...
export function RouteForm({ routeVehicles, routeInstitutions, institutionsLoading = false, institutionsError, onClose, onCreated }: { routeVehicles: Array<{ id: number; brand: string; model: string; licensePlate: string }>; routeInstitutions: Institution[]; institutionsLoading?: boolean; institutionsError?: string; onClose: () => void; onCreated: () => void }) {
  ...
  function submit(event: FormEvent) {
    ...
    if (!schedules.length) return setValidationError('Adicione uma ida e uma volta para pelo menos um dia.')
    if (schedules.some(item => !item.departureTime)) return setValidationError('Defina todos os horários de ida e volta adicionados.')
    if (days.some(([day]) => { const daySchedules = schedules.filter(item => item.dayOfWeek === day); return daySchedules.length > 0 && (daySchedules.length !== 2 || !daySchedules.some(item => item.direction === 'IDA') || !daySchedules.some(item => item.direction === 'VOLTA')) })) return setValidationError('Cada dia selecionado precisa ter uma ida e uma volta.')
  ...
  return <div className="modal-backdrop route-modal-backdrop"><form className="route-form" onSubmit={submit}><div className="section-heading"><div><p className="section-kicker">Nova rota</p><h2>Monte sua rotina</h2></div><button type="button" className="text-button" onClick={onClose}>Fechar</button></div><p className="muted">Comece pelo seu horário de saída. A prévia e as confirmações serão geradas a partir daqui.</p>
    ...
    <fieldset className="route-fieldset"><legend>Saídas recorrentes</legend><p className="muted">Cada rota tem uma ida e uma volta por dia. Para outro horário no mesmo dia, crie uma nova rota.</p>{days.map(([day, label]) => { const daySchedules = schedules.filter(item => item.dayOfWeek === day); return <div className="route-day" key={day}><strong>{label}</strong><div className="route-day-content"><div className="route-day-schedules">{daySchedules.map(item => <div className="route-time-card" key={item.id}><div className="route-time-card-heading"><strong>{item.direction === 'IDA' ? 'Ida' : 'Volta'}</strong><button type="button" className="remove-slot" aria-label={`Remover ${item.direction === 'IDA' ? 'ida' : 'volta'} de ${label}`} onClick={() => setSchedules(current => current.filter(value => value.id !== item.id))}>Remover</button></div><TimeButton label="Horário de saída" value={item.departureTime} onClick={() => setTimePicker({ scheduleId: item.id, label: `${label} · ${item.direction === 'IDA' ? 'Ida' : 'Volta'}` })} /></div> )}</div><div className="route-add-actions">{!daySchedules.some(item => item.direction === 'IDA') && <button type="button" className="add-slot" onClick={() => addSchedule(day, 'IDA')}>+ Ida</button>}{!daySchedules.some(item => item.direction === 'VOLTA') && <button type="button" className="add-slot" onClick={() => addSchedule(day, 'VOLTA')}>+ Volta</button>}</div></div></div> })}</fieldset>
